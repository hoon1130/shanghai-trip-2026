...
def update_shanghai_app():
    ...
    if 's.subway ?' not in content:
        # Looking for the place where we append pinyin/nameZh
    ...
    # Fix the init line where multiple functions were concatenated wrongly in previous turns
    # The problematic line usually starts with fetchWeather() or has duplicated loadItinerary()
    init_pattern = re.compile(r'fetchWeather\(\);.*?;', re.DOTALL)
    clean_init = "fetchWeather(); renderPhrases(); loadItinerary(); loadExpenses(); loadChecklist(); loadSpots();"
    content = init_pattern.sub(clean_init, content)
