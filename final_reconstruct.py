...
def final_reconstruct():
    ...
    # 1. Complete Spot List with updated subway format
    ...
    content = re.sub(r'const initialSpots = \[.*?\];', new_spots_data, content, flags=re.DOTALL)

    # 2. Re-write the renderSpots function completely to avoid template string interpolation errors
    ...
    # We must replace from the start of renderSpots to the end of openMetro or whatever follows.
    # Identifying the full block to replace
    pattern = re.compile(r'function renderSpots\(\) \{.*?\}', re.DOTALL)
    content = pattern.sub(render_spots_code, content)
    
    # Check if there is any duplicated openMetro and remove it
    content = re.sub(r'function openMetro\(keyword\) \{.*?\}', '', content, flags=re.DOTALL)
    # Put it back correctly by including it in the render_spots_code replacement above.
    # Wait, the re.sub(renderSpots) already includes openMetro.

    # 3. Clean up the faulty loadSpots logic (Ensure it doesn't duplicate)
