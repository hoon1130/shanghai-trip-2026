...
<body class="bg-slate-50 dark:bg-slate-950 pb-24 transition-colors duration-300 text-slate-900 dark:text-slate-100">
    ...
    <main class="p-4">
        ...
        <div id="schedule" class="tab-content active space-y-5">
            ...
            <div id="family-itinerary" class="space-y-4">
                <div class="flex justify-between items-center px-1">
                    ...
                    <button onclick="openItineraryForm()" class="bg-slate-900 dark:bg-brand-500 text-white px-4 py-2 rounded-lg text-[11px] font-black shadow-lg active:scale-95 transition-transform"><i class="fas fa-plus mr-1"></i>일정 추가</button>
                </div>
        ...
        <div id="ledger" class="tab-content space-y-5">
            <div class="bg-slate-900 text-white p-7 rounded-[2.5rem] shadow-xl relative overflow-hidden">
                ...
                <div class="space-y-4 relative z-10 mt-6">
                    ...
                    <div class="flex gap-2 mb-2">
                        <button onclick="addCNY(10)" class="flex-1 bg-white/10 hover:bg-white/20 text-white py-3 rounded-xl font-black text-xs active:scale-95">+10¥</button>
                        <button onclick="addCNY(50)" class="flex-1 bg-white/10 hover:bg-white/20 text-white py-3 rounded-xl font-black text-xs active:scale-95">+50¥</button>
                        <button onclick="addCNY(100)" class="flex-1 bg-white/10 hover:bg-white/20 text-white py-3 rounded-xl font-black text-xs active:scale-95">+100¥</button>
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-md">
                ...
                <div class="flex gap-2 mb-6">
                    ...
                    <button onclick="addExpense()" class="px-4 bg-slate-900 dark:bg-brand-500 text-white rounded-xl font-black text-sm active:scale-95 transition-transform">저장</button>
                </div>
        ...
        <div id="phrases" class="tab-content space-y-6">
            <div class="bg-indigo-600 text-white p-6 rounded-[2rem] shadow-lg mb-4 relative overflow-hidden">
                ...
                <p class="text-sm opacity-90 font-bold mt-1 leading-relaxed">번역기를 쓰거나 회화를 들어보세요.</p>
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-md space-y-4">
                <div class="flex justify-between items-center px-1">
                    <h3 class="font-black text-brand-500 italic uppercase tracking-tighter flex items-center text-sm"><i class="fas fa-magic mr-2"></i>In-App Translator</h3>
                    <button onclick="window.open('https://papago.naver.com/?sk=auto&tk=ko&hn=0&st=', '_blank')" class="text-[10px] font-black text-slate-400 underline flex items-center"><i class="fas fa-camera mr-1"></i>Camera</button>
                </div>
                <div class="space-y-3">
                    <div class="relative">
                    ...
                    <div id="trans-result-box" class="hidden p-5 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 relative">
                        ...
                        <button id="trans-speak-btn" class="absolute right-4 top-1/2 -translate-y-1/2 text-indigo-400 hidden"><i class="fas fa-volume-up text-xl"></i></button>
                    </div>
                </div>
            </div>
    ...
    <div id="sos-modal" class="fixed inset-0 bg-black/70 z-[400] hidden flex items-center justify-center p-5 backdrop-blur-md">
        <div class="bg-white dark:bg-slate-900 p-7 rounded-[2.5rem] w-full max-w-md space-y-5 shadow-2xl border-t-[8px] border-red-500">
            <div class="flex justify-between items-center border-b pb-4 dark:border-slate-800">
                <h3 class="font-black text-2xl text-red-500 uppercase italic tracking-tighter">SOS</h3>
            ...
            <button onclick="showHotelCard()" class="w-full bg-slate-900 dark:bg-brand-500 text-white py-5 rounded-2xl font-black text-lg active:scale-95 transition-transform flex flex-col items-center">
                ...
                <span class="text-[10px] opacity-70 uppercase tracking-widest mt-1">Show to driver</span>
            </button>
    ...
    <div id="hotel-card" class="fixed inset-0 bg-white z-[500] hidden flex flex-col items-center justify-center p-8 text-center" onclick="this.classList.add('hidden')">
        <p class="text-slate-500 font-black text-xl mb-10">기사님, 여기로 가주세요 (请帶我去这里)</p>
        ...
        <div class="bg-slate-100 px-6 py-3 rounded-full font-black text-slate-400 animate-pulse">Touch to close</div>
    </div>
    ...
    <div id="itinerary-modal" class="fixed inset-0 bg-black/60 z-[100] hidden flex items-center justify-center p-5 backdrop-blur-sm">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-md space-y-5 shadow-2xl">
            <h3 class="font-black text-2xl border-b pb-3 dark:border-slate-800 text-brand-500 text-center uppercase italic">Edit Plan</h3>
            <input type="hidden" id="edit-key">
            <select id="form-date" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            ...
            <input type="text" id="form-time" placeholder="시간 (예: 14:30)" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            <input type="text" id="form-place" placeholder="장소명" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            <textarea id="form-memo" placeholder="상세 내용" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black h-32 outline-none"></textarea>
            <div class="flex space-x-3"><button onclick="saveItinerary()" class="flex-1 bg-slate-900 dark:bg-brand-500 text-white py-4 rounded-2xl font-black text-lg">SAVE</button><button onclick="closeItineraryModal()" class="flex-1 bg-slate-200 dark:bg-slate-700 py-4 rounded-2xl font-black text-lg">CLOSE</button></div>
            <button id="delete-btn" onclick="deleteItinerary()" class="w-full text-red-500 font-bold text-xs underline mt-4">Delete Itinerary</button>
        </div>
    </div>

    <div id="spot-modal" class="fixed inset-0 bg-black/60 z-[100] hidden flex items-center justify-center p-5 backdrop-blur-sm">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-md space-y-5 shadow-2xl">
            <h3 class="font-black text-2xl border-b pb-3 dark:border-slate-800 text-brand-500 text-center uppercase italic">Spot Info</h3>
            <input type="hidden" id="spot-key"><select id="spot-cat" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none"><option value="핵심">핵심</option><option value="명소">명소</option><option value="맛집">맛집</option><option value="쇼핑/기타">쇼핑/기타</option></select>
            <input type="text" id="spot-name-ko" placeholder="한글 명칭" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            <input type="text" id="spot-name-zh" placeholder="중문 명칭" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            <input type="text" id="spot-subway" placeholder="지하철 정보" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            <input type="text" id="spot-addr" placeholder="상세 주소" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            <div class="flex space-x-3"><button onclick="saveSpot()" class="flex-1 bg-slate-900 dark:bg-brand-500 text-white py-4 rounded-2xl font-black text-lg">저장</button><button onclick="closeSpotModal()" class="flex-1 bg-slate-200 dark:bg-slate-700 py-4 rounded-2xl font-black text-lg">닫기</button></div>
            <button id="spot-delete-btn" onclick="deleteSpot()" class="w-full text-red-500 font-bold text-xs underline mt-4">장소 삭제</button>
        </div>
    </div>
    ...
    <div id="checklist-modal" class="fixed inset-0 bg-black/60 z-[100] hidden flex items-center justify-center p-5 backdrop-blur-sm">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-md space-y-5 shadow-2xl">
            <h3 class="font-black text-2xl border-b pb-3 dark:border-slate-800 text-brand-500 text-center">Item Add</h3>
            <input type="hidden" id="checklist-type"><input type="hidden" id="checklist-key">
            <input type="text" id="checklist-title" placeholder="항목 이름" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-black outline-none">
            <div class="flex space-x-3"><button onclick="saveChecklist()" class="flex-1 bg-slate-900 dark:bg-brand-500 text-white py-4 rounded-2xl font-black text-lg">저장</button><button onclick="closeChecklistModal()" class="flex-1 bg-slate-200 dark:bg-slate-700 py-4 rounded-2xl font-black text-lg">닫기</button></div>
        </div>
    </div>
    ...
    <div id="flashcard" class="fixed inset-0 bg-yellow-400 z-[600] hidden flex flex-col items-center justify-center p-8 text-center" onclick="this.classList.add('hidden')">
        ...
        <p class="mt-12 text-sm text-yellow-800 opacity-50 font-black tracking-widest uppercase">Touch to close</p>
    </div>
...
with codecs.open(path, 'w', encoding='utf-8') as f:
    f.write(full_html)
print("Final Master Pro Build (v34) applied successfully.")
