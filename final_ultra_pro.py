...
<head>
    ...
    <style>
        ...
        .category-chip.active { background-color: #4f46e5; color: white; border-color: #4f46e5; }
...
<body class="bg-slate-50 dark:bg-slate-950 pb-24 transition-colors duration-300 text-slate-900 dark:text-slate-100">
    ...
    <nav class="fixed bottom-0 left-0 right-0 glass-header flex bottom-nav z-50 shadow-2xl">
        <button onclick="showTab('schedule')" class="tab-btn flex-1 flex flex-col items-center justify-center text-brand-500"><i class="fas fa-route text-[22px] mb-1"></i><span class="text-[9px] font-black uppercase tracking-tighter">Plan</span></button>
        <button onclick="showTab('spot')" class="tab-btn flex-1 flex flex-col items-center justify-center text-slate-400"><i class="fas fa-map-marked-alt text-[22px] mb-1"></i><span class="text-[9px] font-black uppercase tracking-tighter">Spot</span></button>
        <button onclick="showTab('ledger')" class="tab-btn flex-1 flex flex-col items-center justify-center text-slate-400"><i class="fas fa-wallet text-[22px] mb-1"></i><span class="text-[9px] font-black uppercase tracking-tighter">Money</span></button>
        <button onclick="showTab('phrases')" class="tab-btn flex-1 flex flex-col items-center justify-center text-slate-400"><i class="fas fa-comment-dots text-[22px] mb-1"></i><span class="text-[9px] font-black uppercase tracking-tighter">Talk</span></button>
        <button onclick="showTab('packing')" class="tab-btn flex-1 flex flex-col items-center justify-center text-slate-400"><i class="fas fa-suitcase text-[22px] mb-1"></i><span class="text-[9px] font-black uppercase tracking-tighter">Box</span></button>
    </nav>
    ...
    <div id="sos-modal" class="fixed inset-0 bg-black/70 z-[400] hidden flex items-center justify-center p-5 backdrop-blur-md">
        <div class="bg-white dark:bg-slate-900 p-7 rounded-[2.5rem] w-full max-w-md space-y-5 shadow-2xl border-t-[8px] border-red-500">
            ...
            <button onclick="showHotelCard()" class="w-full bg-slate-900 dark:bg-brand-500 text-white py-5 rounded-2xl font-black text-lg shadow-lg active:scale-95 transition-transform flex flex-col items-center">
                ...
                <span class="text-[10px] opacity-70 font-bold tracking-widest">Show address to driver</span>
            </button>
            ...
            <button onclick="showHelpPhrase('길을 잃었습니다. 도와주세요.', '我迷路了, 请帮帮我。', '워 미루러, 칭 빵빵워')" class="w-full bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl text-left font-bold text-sm flex justify-between items-center">
    ...
    <div id="hotel-card" class="fixed inset-0 bg-white z-[500] hidden flex flex-col items-center justify-center p-8 text-center" onclick="this.classList.add('hidden')">
        <p class="text-slate-500 font-black text-xl mb-10 text-center">기사님, 여기로 가주세요 (请带我去这里)</p>
        <h2 class="text-7xl font-black text-slate-950 mb-6">上海国金汇</h2>
        <p class="text-3xl font-bold text-slate-700 leading-tight mb-12">上海市浦동新区<br>世纪大道8号</p>
    ...
    <div id="itinerary-modal" class="fixed inset-0 bg-black/60 z-[100] hidden flex items-center justify-center p-5 backdrop-blur-sm">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-md space-y-5 shadow-2xl">
            <h3 class="font-black text-2xl border-b pb-3 dark:border-slate-800 tracking-tighter italic uppercase text-brand-500 text-center">Edit Plan</h3>
            <input type="hidden" id="edit-key"><select id="form-date" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none"><option value="05-05(화)">5월 5일 (화)</option><option value="05-06(수)">5월 6일 (수)</option><option value="05-07(목)">5월 7일 (목)</option><option value="05-08(금)">5월 8일 (금)</option><option value="05-09(토)">5월 9일 (토)</option></select>
            <input type="text" id="form-time" placeholder="시간" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none">
            <input type="text" id="form-place" placeholder="장소명" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none">
            <textarea id="form-memo" placeholder="상세 내용" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black h-32 text-base outline-none"></textarea>
            <div class="flex space-x-3 pt-2"><button onclick="saveItinerary()" class="flex-1 bg-slate-900 dark:bg-brand-500 text-white py-4 rounded-2xl font-black text-lg active:scale-95 transition-transform">SAVE</button><button onclick="closeItineraryModal()" class="flex-1 bg-slate-200 dark:bg-slate-700 py-4 rounded-2xl font-black text-lg active:scale-95 transition-transform">CLOSE</button></div>
            <button id="delete-btn" onclick="deleteItinerary()" class="w-full text-red-500 font-bold text-xs underline hidden uppercase tracking-widest mt-4 text-center">Delete Itinerary</button>
        </div>
    </div>

    <div id="spot-modal" class="fixed inset-0 bg-black/60 z-[100] hidden flex items-center justify-center p-5 backdrop-blur-sm">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-md space-y-5 shadow-2xl">
            <h3 id="spot-modal-title" class="font-black text-2xl border-b pb-3 dark:border-slate-800 tracking-tighter italic uppercase text-brand-500 text-center">장소 정보</h3>
            <input type="hidden" id="spot-key"><select id="spot-cat" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none"><option value="핵심">핵심</option><option value="명소">명소</option><option value="맛집">맛집</option><option value="쇼핑/기타">쇼핑/기타</option></select>
            <input type="text" id="spot-name-ko" placeholder="한글 명칭" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none">
            <input type="text" id="spot-name-zh" placeholder="중문 명칭" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none">
            <input type="text" id="spot-subway" placeholder="지하철 정보" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none">
            <input type="text" id="spot-addr" placeholder="상세 주소" class="w-full p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border-none font-black text-base outline-none">
            <div class="flex space-x-3 pt-2"><button onclick="saveSpot()" class="flex-1 bg-slate-900 dark:bg-brand-500 text-white py-4 rounded-2xl font-black text-lg active:scale-95 transition-transform">저장</button><button onclick="closeSpotModal()" class="flex-1 bg-slate-200 dark:bg-slate-700 py-4 rounded-2xl font-black text-lg active:scale-95 transition-transform">닫기</button></div>
            <button id="spot-delete-btn" onclick="deleteSpot()" class="w-full text-red-500 font-bold text-xs underline hidden uppercase tracking-widest mt-4 text-center">장소 삭제</button>
        </div>
    </div>

    <div id="tip-modal" class="fixed inset-0 bg-black/80 z-[200] hidden p-6 flex items-center justify-center backdrop-blur-md">
        <div class="bg-white dark:bg-slate-900 p-8 rounded-[3rem] w-full max-w-md flex flex-col max-h-[80vh] shadow-2xl border-t-[10px] border-amber-400">
    ...
    <div id="flashcard" class="fixed inset-0 bg-yellow-400 z-[600] hidden flex flex-col items-center justify-center p-8 text-center" onclick="this.classList.add('hidden')">
        <h2 id="flash-zh" class="text-6xl font-black text-slate-900 mb-8 leading-tight"></h2>
        <p id="flash-ko" class="text-3xl font-bold text-yellow-900 mb-2"></p>
        <p id="flash-py" class="text-2xl font-bold text-yellow-700 mb-10"></p>
        <p class="mt-12 text-sm text-yellow-800 opacity-50 font-black tracking-widest uppercase animate-pulse">Touch to close</p>
    </div>
    ...
    <script>
        ...
        const initialSpots = [
            ...
            {cat: "맛집", nameKo: "점도덕 (신천지)", nameZh: "点도德(新天地店)", addr: "", subway: "新天地(Xin Tian Di) 신천지역 | 2번출구 도보 5분", lat: 31.216, lng: 121.479},
        ...
        function loadExpenses() {
            ...
            db.ref(`${basePath}/expenses`).on('value', s => {
                const data = s.val() || {}; let tC = 0, tK = 0; const cats = {'식비':0,'교통':0,'쇼핑':0,'기타':0}, colors = {'식비':'bg-orange-400','교통':'bg-blue-400','쇼핑':'bg-purple-400','기타':'bg-slate-400'};
                const listHtml = Object.entries(data).map(([k, v]) => {
                    ...
                    return `<div class="flex justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 text-xs mb-2 shadow-sm"><div class="flex flex-col"><div class="flex items-center"><span class="w-2 h-2 rounded-full ${colors[cName]||'bg-slate-400'} mr-2"></span><span class="font-bold">${v.desc}</span></div><span class="text-[10px] text-slate-400 ml-4">${kAmt.toLocaleString()} ₩</span></div><span class="font-black text-indigo-500 text-sm">${v.amt.toLocaleString()} ¥ <button onclick="db.ref('${basePath}/expenses/${k}').remove()" class="ml-3 text-slate-300">&times;</button></span></div>`;
                }).join('');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            fetchWeather(); loadItinerary(); loadExpenses(); loadChecklist(); loadSpots();
        });
    </script>
</body>
</html>"""
...
with codecs.open(path, 'w', encoding='utf-8') as f:
    f.write(full_html)
print("Final Ultra Master Pro (v33) fully applied.")
