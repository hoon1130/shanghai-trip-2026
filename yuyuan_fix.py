...
def final_yuyuan_fix():
    with codecs.open(path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. Comprehensive list of initial spots including Yuyuan variants and Chain stores
    # Format: 한자(Pinyin) 한글역명 | 출구, 도보 거리/시간
    ...
    # 2. Update the sync logic to handle nameKo matching as fallback and ensure subway info updates
    migration_pattern = re.compile(r'initialSpots\.forEach\(init => \{.*?\}\);', re.DOTALL)
    new_migration = """initialSpots.forEach(init => {
                        // Match by nameZh OR nameKo to catch Yuyuan garden variants
                        const existingItem = spotData.find(item => item.nameZh === init.nameZh || item.nameKo === init.nameKo);
                        if (!existingItem) {
                            db.ref(`${basePath}/spots`).push(init);
                        } else {
                            // Update subway and addr to ensure latest info and URL cleanup
    ...
    content = migration_pattern.sub(new_migration, content)
