...
def update_spots_final():
    with codecs.open(path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 1. Update initialSpots with detailed info (Hanzi, distance, time) and empty addr
    ...
    # 2. Update UI: Larger subway text + Clickable link to metro map
    ...
    # We need to be careful with the replacement. Let's find the current block.
    # Current block has the ${s.subway ? ...} pattern.
    content = re.sub(r'\$\{s\.subway \? .*? : \'\'\}</div>', render_spots_new, content)

    # 3. Update Migration Logic: Force sync info for existing items
    migration_old = """if (!existingItem) {
