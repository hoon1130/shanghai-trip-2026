...
def update_shanghai_app():
    ...
    # Using \u00B0 instead of degree symbol to avoid encoding issues
    ...
    # Ensuring all required functions are called in DOMContentLoaded without duplication or syntax errors
    init_match = re.search(r'document\.addEventListener\(\'DOMContentLoaded\', \(\) => \{.*?\}', content, re.DOTALL)
    if init_match:
        old_init = init_match.group(0)
        # Reconstruct a clean DOMContentLoaded body
